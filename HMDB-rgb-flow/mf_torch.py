...
def get_frames_gpu(sample, device):
    arr = sample['video'].to(device)            # [1,T,3,H,W] or [T,3,H,W]
    if arr.dim()==5 and arr.shape[0]==1:
        arr = arr[0]
    if arr.dim()==4 and arr.shape[-1]==3:
        arr = arr.permute(0,3,1,2)
    return arr.float()/255.0

def get_flow_gpu(sample, device):
    arr = sample['video'].to(device)            # [1,T-1,C,H,W] or [T-1,C,H,W]
    if arr.dim()==5 and arr.shape[0]==1:
        arr = arr[0]
    if arr.dim()==4 and arr.shape[-1] in (2,3):
        arr = arr.permute(0,3,1,2)
    if arr.shape[1]==3:
        arr = arr[:,:2]
    return arr.float()
...
if __name__ == '__main__':
    ...
    for dataset in dataset_lst:
        ...
        ds = EPICDOMAIN(
            ...
            datapath=d_path[dataset], # ucf debug
            dataset= dataset,
            ood_dataset=dataset, # ucf debug
            near_ood=near_ood,
            far_ood=False
        )
        ...
        np.save(f'meta_features_{dataset}_id.npy', X)
        np.save(f'meta_labels_{dataset}_id.npy', y)
