...
if __name__ == '__main__':
    ...
    for dataset in dataset_lst:
        ...
        ds = EPICDOMAIN(split='train', eval=True, cfg=model.module.cfg, cfg_flow=mflow.module.cfg, datapath='/data/shared_dataset/data/EPIC_KITCHENS/', far_ood=False)
        loader = DataLoader(ds, batch_size=1, shuffle=False,
                            num_workers=16, pin_memory=True)

        X, y = build_meta_vectors(loader, device)
        np.save(f'meta_features_EPIC_id.npy', X)
        np.save(f'meta_labels_EPIC_id.npy', y)
